...
class DynamicsCrm {
	...
	private function call($operation, $soapBody,$page=1,$Return =false) {
		...
		if ($response === false) {
			...
			$BoolMoreRecords=false;
				...
				$Return->ErrorMessage .='<br>D�tail : '.$this->getErrorMessage($xml->Body->Fault->detail->OrganizationServiceFault);
				};
				$Return->Result = False;
			} else {
   ...
   private function GetCrmError($xml){
			$CrmError=new CrmResponse();
		   	...
		   	if (isset($xml->Body->Fault->detail->OrganizationServiceFault)){
		   		$CrmError->ErrorMessage .='<br>Détail : '.$this->getErrorMessage($xml->Body->Fault->detail->OrganizationServiceFault);
		   	};
